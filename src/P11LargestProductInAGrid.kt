 ...
 */
private fun findSolution(arr: Array<Array<Int>>): Long {
    var max = 0L
    for (row in arr.indices) {
        for (col in arr[row].indices) {
            max = max(gridMax(arr, row, col, 4), max)
        }
    }
...
private fun gridMax(a: Array<Array<Int>>, startRow: Int, startCol: Int, numberOfElements: Int): Long {
    var productRight = 1L
    var productDown = 1L
    var productDiagonalLeft = 1L
    var productDiagonalRight = 1L

    for (n in 0 until numberOfElements) {
        productRight *= a.el(startRow, startCol + n)
        productDown *= a.el(startRow + n, startCol)
        productDiagonalLeft *= a.el(startRow + n, startCol + n)
        productDiagonalRight *= a.el(startRow + n, startCol - n)
    }
    return max(productDiagonalLeft, productDiagonalRight, productDown, productRight)
}
...
fun Array<Array<Int>>.el(row: Int, col: Int): Long {
    if (row < 0 || row >= this.size) {
        return 0L
    } else if (col < 0 || col >= this[row].size) {
        return 0L
    }

    return this[row][col].toLong()
}
